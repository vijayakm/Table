...
describe('TableColumn Component', () => {
  it('renders the column label', () => {
    const { getByText } = render(<table><thead><tr><TableColumn {...mockProps} /></tr></thead></table>);
  ...
  it('does not display sorting icon for non-sorted columns', () => {
    const { getByText, queryByAltText } = render(<table><thead><tr><TableColumn {...mockProps} /></tr></thead></table>);
    expect(getByText('Column Label')).toBeInTheDocument();
  ...
  it('does not call onSort for non-sortable columns', () => {
    const { getByText } = render(<table><thead><tr><TableColumn {...mockProps} sortable={false} /></tr></thead></table>);
    fireEvent.click(getByText('Column Label'));
